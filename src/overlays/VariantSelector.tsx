...
export const VariantSelector: React.FC<VariantSelectorProps> = ({
  ...
  const handleSelectCase = (newCase: string) => {
    ...
    if (exactMatch) {
      setSelectedColor({ ...exactMatch, color: formatIWatchColor(exactMatch.color) });
      return;
    }
    ...
    if (matchCase) {
      setSelectedColor({ ...matchCase, color: formatIWatchColor(matchCase.color) });
    }
  };
  ...
  const handleSelectStrap = (newStrap: string) => {
    ...
    if (matchStrap) {
      setSelectedColor({ ...matchStrap, color: formatIWatchColor(matchStrap.color) });
    }
  };

  return (
    <BottomSheet isOpen={isOpen} onClose={onClose} title="Select Options" id="variant-selector-sheet">
      <div className="space-y-5 pb-2">
        {/* Product summary snippet */}
        <div className="flex gap-4 items-center pb-4 border-b border-gray-100">
          <div className="w-18 h-18 bg-[#fbfbfd] rounded-2xl border border-black/5 p-2 flex items-center justify-center flex-shrink-0">
            {activeImage && (
              <img
                src={activeImage}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] text-[#86868b] mt-0.5 truncate">
              Selected: {formattedSelectedColor} • {selectedSize.size}
            </div>
          </div>
        </div>
