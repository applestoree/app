...
const reverseNearest = async (lat: number, lon: number, zoom: number): Promise<NominatimNearestResponse | null> => {
  try {
    ...
    const response = await fetch(url, {
      headers: { 'Accept-Language': 'en-MY, en;q=0.9, ms-MY;q=0.8, ms;q=0.7' },
    });
...
export const DeliveryAddressPage: React.FC = () => {
  ...
  const handleAddressResolved = async (resolved: ResolvedAddressResult) => {
    ...
    setAddress((prev) => ({
      ...prev,
      street: completed.street || prev.street,
      city: completed.city || prev.city,
      state: completed.state || prev.state,
      postcode: completed.postcode || prev.postcode,
      country: 'Malaysia',
    }));
  };
  ...
  const handleSave = () => {
    if (!hasAddress) {
      alert('Please select a complete delivery address from the map');
      return;
    }
      ...
      footer={
        <div className="w-full bg-white/95 backdrop-blur-md border-t border-black/5 p-3 px-4 shrink-0">
          <button
            ...
            onClick={handleSave}
            disabled={!hasAddress}
            className="w-full p-3 bg-[#0071e3] disabled:bg-gray-300 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-xl"
          >
            Save Address
          </button>
        </div>
      ...
      <div className="flex-1 p-4 pb-6 space-y-3">
        <section className="bg-white p-4 rounded-2xl border border-black/5 space-y-2">
          ...
          <input
            ...
            readOnly
            placeholder="Address"
            className="w-full p-3 rounded-xl border text-xs bg-[#f5f5f7] text-[#1d1d1f]"
          />
        </section>
