...
export const AuthPage: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try { mode === 'login' ? await login(phone.trim(), password) : await register(name.trim(), phone.trim(), password); navigate('/profile'); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to continue'); }
    finally { setLoading(false); }
  };
  return <StandalonePage title={mode === 'login' ? 'Sign In' : 'Create Account'}><form onSubmit={submit} className="p-4 space-y-3">
