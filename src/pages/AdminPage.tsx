...
export function AdminPage() {
  ...
  const createProduct = async () => { try { const value = formToProduct(form); if (!value.item_group_id) throw new Error('Item Group ID is required'); await adminApi.createProduct(value); setForm(emptyForm()); setEditing(false); await load(); } catch (e) { setError(e instanceof Error ? e.message : 'Create failed'); } };
  const updateProduct = async () => { try { const value = formToProduct(form); if (!value.item_group_id) throw new Error('Item Group ID is required'); await adminApi.updateProduct(value.item_group_id, value); setEditing(false); await load(); } catch (e) { setError(e instanceof Error ? e.message : 'Update failed'); } };
  const editProduct = (p: any) => { setForm(productToForm(p)); setEditing(true); setView('products'); };
  ...
  return <main className="w-full max-w-[500px] mx-auto h-full min-h-[100dvh] bg-gray-50 flex flex-col">
    ...
    {productPage && <div className="w-full shrink-0 border-t border-gray-200 bg-white p-3"><div className="flex gap-2"><button type="button" onClick={() => void createProduct()} disabled={busy} className="flex-1 rounded-xl bg-black px-3 py-3 text-sm font-medium text-white disabled:opacity-50">Create</button><button type="button" onClick={() => void updateProduct()} disabled={busy || !editing} className="flex-1 rounded-xl border border-gray-200 px-3 py-3 text-sm font-medium disabled:opacity-50">Update</button></div></div>}
