...
function OrdersView({ orders, onStatusChange, onPaymentChange }: { orders: any[]; onStatusChange: (id: number | string, status: string) => Promise<void>; onPaymentChange: (id: number | string, payment: any) => Promise<void> }) {
  ...
  const selectedOrder = selectedOrderId == null ? null : orders.find(order => String(order.id) === selectedOrderId) || null;
  ...
  return <Section title="Orders"><div className="flex items-end justify-between"><div><p className="text-sm text-gray-500">{orders.length} order{orders.length === 1 ? '' : 's'}</p></div></div>
    {orders.length === 0 ? <p className="text-sm text-gray-500">No orders.</p> : <div className="space-y-3">{orders.map(o => {
      ...
      return <button key={String(o.id)} type="button" onClick={() => setSelectedOrderId(String(o.id))} className="block w-full rounded-2xl border border-gray-200 bg-white p-4 text-left transition hover:border-gray-300 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-300">
