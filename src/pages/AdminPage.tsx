...
export function AdminPage() {
  ...
  return <main className="w-full max-w-[500px] mx-auto h-full min-h-[100dvh] bg-gray-50 flex flex-col">
    {productPage && <header className="w-full shrink-0 border-b border-gray-200 bg-white px-4 py-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs text-gray-500">Admin / Products</p><h1 className="text-lg font-semibold text-gray-900">{productCrudOpen ? 'Product CRUD' : 'Products'}</h1></div><div className="flex items-center gap-2">{productCrudOpen ? <><button type="button" onClick={() => { setForm(emptyForm()); setEditing(false); }} className="rounded-xl border border-gray-200 px-3 py-2 text-sm">New</button><button type="button" onClick={() => setProductCrudOpen(false)} className="rounded-xl border border-gray-200 px-3 py-2 text-sm">Hide</button></> : <button type="button" onClick={() => { setForm(emptyForm()); setEditing(false); setProductCrudOpen(true); }} className="rounded-xl bg-black px-3 py-2 text-sm text-white">New Product</button>}<button type="button" onClick={logout} className="rounded-xl border border-red-200 px-3 py-2 text-sm text-red-600">Logout</button></div></div></header>}
    ...
    {productPage && productCrudOpen && <div className="w-full shrink-0 border-t border-gray-200 bg-white p-3"><div className="flex gap-2"><button type="button" onClick={() => void createProduct()} disabled={busy} className="flex-1 rounded-xl bg-black px-3 py-3 text-sm font-medium text-white disabled:opacity-50">Create</button><button type="button" onClick={() => void updateProduct()} disabled={busy || !editing} className="flex-1 rounded-xl border border-gray-200 px-3 py-3 text-sm font-medium disabled:opacity-50">Update</button></div></div>}
    {(!productPage || !productCrudOpen) && <div className="w-full shrink-0"><AdminBottomNav activeView={view} onViewChange={setView} /></div>}
  </main>;
}
